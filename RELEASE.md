...
- [ ] Ensure release candidates are being successfully built (see `conda-canary/label/conda-pycosat-rc-MAJOR.MINOR.x`)
...
Once the release PRs are filed, successful canary builds will be available on `https://anaconda.org/conda-canary/pycosat/files?channel=conda-pycosat-rc-MAJOR.MINOR.x` for manual testing.

> [!NOTE]
> You do not need to apply the `build::review` label for release PRs; every commit to the release branch builds and uploads canary builds to the respective `conda-pycosat-rc-MAJOR.MINOR.x` label.
...
2. Clone and `cd` into the repository if you haven't done so already:
    ...
    ```bash
    $ git clone git@github.com:/pycosat.git
    $ cd pycosat
    ```
...
2. Create a versioned branch, this is where rever will make its changes:
    ...
    ```bash
    $ git checkout -b changelog-MAJOR.MINOR.MINOR
    ```
...
2. Run `rever --activities authors <VERSION>`:
    ...
    - Here's a sample run where we undo the commit made by rever in order to commit the changes to `.authors.yml` separately:
        ...
        ```bash
        $ rever --activities authors --force MAJOR.MINOR.MINOR
...
8. Push this versioned branch.
    ...
    ```bash
    $ git push -u upstream changelog-MAJOR.MINOR.MINOR
    ```
...
To publish the release, go to the project's release page (e.g., https://github.com/conda/pycosat/releases) and add the release notes from `CHANGELOG.md` to the draft release you created earlier. Then publish the release.
...
> [!NOTE]
> Make sure NOT to push the "Update Branch" button. If there are [merge conflicts][merge conflicts], resolve them by merging `main` into the `MAJOR.MINOR.x` branch, then push the resulting merge commit. This is now a "bridging branch" to resolve the merge conflict without placing changes from `main` on to the `MAJOR.MINOR.x` branch.
